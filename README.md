...
- Use zeroconf to publish server info.
    ...
    - Usage example: add `-H h -b SERVER-IP-ADDR` command line option, then you can access the server using `http://h.local:8000` (default port number).
    - Depends on [python-zeroconf](https://pypi.org/project/zeroconf/) . You don't need to install it if you do not use this functionality.
